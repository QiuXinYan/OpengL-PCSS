...
//PCSS light
uniform int BLOCKER_SEARCH_NUM_SAMPLES ;  //64
uniform int PCF_NUM_SAMPLES ;//64;
uniform float NEAR_PLANE ;//0.05;
uniform float LIGHT_WORLD_SIZE ;// .5;
uniform float LIGHT_FRUSTUM_WIDTH; //3.75; 
uniform float LIGHT_SIZE; //(LIGHT_WORLD_SIZE / LIGHT_FRUSTUM_WIDTH);
...
{
    float blockerSum = 0;
    float searchWidth = LIGHT_SIZE * (zReceiver - NEAR_PLANE) / zReceiver;
    for(int i = 0; i < BLOCKER_SEARCH_NUM_SAMPLES; i++) {
        vec2 offset = Poisson64[i];
        float sampleDepth = texture(shadowMap, uv + offset *  searchWidth).r;
...
float PCSS(vec2 uv, float bias, float zReceiver) {
    //TODO
    // STEP 1: blocker search
    float avgBlockerDepth = 0.0f;
    int numBlockers = 0;
    FindBlocker(avgBlockerDepth, numBlockers, uv, bias, zReceiver);
...
float ShadowCalculation(float bias)
{
    ...
    return PCSS(projCoords.xy, bias, currentDepth);
}
...
{           
    ...
    float bias = max(0.09 * (1.0 - dot(normal, lightDir)), 0.001);
    // calculate shadow    
    float shadow = ShadowCalculation(bias);
    vec3 lighting = ambient + (1- shadow) * (specular + diffuse);
